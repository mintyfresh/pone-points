...
RSpec.describe Webhooks::MembershipDestroyWebhookSubscriber, type: :subscriber do
  subject(:subscriber) { described_class.new(event, payload) }

  let(:event) { 'app.memberships.destroy' }
  ...
  describe '#perform' do
    ...
    it 'matches the webhook JSON schema' do
      expect { perform }.to have_enqueued_job(DeliverWebhookJob).once.with(
        webhook, match_schema('webhooks/app.memberships.destroy')
      )
    end
  end
end
